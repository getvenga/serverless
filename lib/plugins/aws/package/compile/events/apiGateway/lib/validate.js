  ...
  getRequest(http) {
    if (http.request) {
      ...
      if (request.parameters) {
        const { rawParameters, parameters } = this.getRequestParameters(request);
        request.parameters = parameters;
        request.rawParameters = rawParameters;
      }
  ...
  getRequestParameters(httpRequest) {
    ...
    _.each(locations, location => {
      ...
      _.each(httpRequest.parameters[location], (value, key) => {
        const isRequired = typeof value === 'object' ? value.required : value;
        parameters[`method.request.${singular}.${key}`] = isRequired;
        rawParameters[`method.request.${singular}.${key}`] = value;
      });
    });
