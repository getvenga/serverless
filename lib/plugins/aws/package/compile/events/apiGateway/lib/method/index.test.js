...
describe('#compileMethods()', () => {
  ...
  it('should support AWS integration type', () => {
    ...
    testEvent.http.request.rawParameters = testEvent.http.request.parameters;
    awsCompileApigEvents.validated.events = [
      testEvent
    ];
    return awsCompileApigEvents.compileMethods().then(() => {
  ...
  it('should map parameters that are mapped', () => {
    const testEvent = {
      functionName: 'First',
      http: {
        path: 'users/create',
        ...
        request: {
          ...
          parameters: {
            ...
            'method.request.header.foo': true,
            'method.request.header.bar': false,
            'method.request.header.complex': false,
          },
        },
      },
    };
    testEvent.http.request.rawParameters = testEvent.http.request.parameters;
    testEvent.http.request.rawParameters['method.request.header.complex'] = { required: false, mappedValue: 'context.domainName' };
    awsCompileApigEvents.validated.events = [
      testEvent
    ];
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        ...
        'integration.request.header.foo': 'method.request.header.foo',
        'integration.request.header.bar': 'method.request.header.bar',
        'integration.request.header.complex': 'context.domainName'
      });
    });
  });
