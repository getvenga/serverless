...
module.exports = {
  ...
  getIntegrationRequestParameters(http) {
    const parameters = {};
    if (http.request && http.request.rawParameters) {
      _.each(http.request.rawParameters, (value, key) => {
        const paramValue = typeof value === 'object' ? value.mappedValue : key;
        parameters[`integration.${key.substring('method.'.length)}`] = paramValue;
      });
    }
